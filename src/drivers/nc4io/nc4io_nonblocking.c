 ...
 * ids[nused:nalloc] => available (unused) request ids
 */
int nc4io_req_list_init(NC_nc4_req_list *lp) {
    int i;

    /* Initialize parameter and allocate the array  */
    lp->nused = 0;
    lp->nalloc = PUT_ARRAY_SIZE;
    lp->reqs = (NC_nc4_req*)NCI_Malloc(lp->nalloc * sizeof(NC_nc4_req));
 ...
 */
static int nc4io_req_list_resize(NC_nc4_req_list *lp)
{
    int i;
    size_t nsize;
    void *ptr;
    ...
    ptr = NCI_Realloc(lp->reqs, nsize * sizeof(NC_nc4_req));
    if (ptr == NULL) DEBUG_RETURN_ERROR(NC_ENOMEM);
    lp->reqs = (NC_nc4_req*)ptr;
 ...
 */
int nc4io_req_list_free(NC_nc4_req_list *lp)
{
    NCI_Free(lp->reqs);
    NCI_Free(lp->ids);
    NCI_Free(lp->pos);
 ...
 */
int nc4io_req_list_add(NC_nc4_req_list *lp, int *id)
{
    int err;

    /* Increase size if necessary */
    if (lp->nused == lp->nalloc) {
        err = nc4io_req_list_resize(lp);
 ...
 * ids[nused:nalloc] => available (unused) request ids
 */
int nc4io_req_list_remove(NC_nc4_req_list *lp, int reqid) {
    NC_nc4_req * req = lp->reqs + reqid;
